<template>
  <div class="calculator-wrapper" @click="calculateHeight">
    <CalculatorToggle :style="{ paddingBottom: show ? 30 : 15 }" @click="showCalc" />
    <transition name="show">
      <div class="calcWithCompoundList" v-if="show" :style="{ maxHeight: height + 'px' }">
        <div class="calculator">
          <ChoiceOverlay v-if="chooseElement" />
          <div class="calcInner" :style="{ filter: chooseElement ? 'blur(10px)' : '' }">
            <Compound />
            <transition-group name="calc-list" appear tag="div" class="calcChoose">
              <CalculatorChoose
                v-for="n in noa"
              ...
              />
              <div class="line" v-if="noa > 2" @click="subtractAtom"></div>
              <div @click="addAnotherAtom" v-if="noa < 5" class="plus"></div>
            </transition-group>
          </div>
        </div>
