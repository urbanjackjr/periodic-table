  ...
  <div class="compound-wrapper">
    <div class="compound">
      <span v-for="n in noa" :key="n"
        ...
        }}<sub>{{ elementQuantity[n] > 1 ? elementQuantity[n] : "" }}</sub></span
      >
    </div>
    <div class="info">
      <h3>m<sub>a</sub> = <Count :number="compoundAtomicMass" /></h3>
      <button class="showCompoundInfo" @click="getSingleData; showCompoundInfo()">
  ...
  data() {
    return {
      ro: new ResizeObserver((entries) => {
        document.querySelector(".calculator") ? this.$store.commit("maxHeight") : "";
      }),
    };
  },
  methods: {
